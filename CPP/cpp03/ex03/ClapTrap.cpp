...
ClapTrap::ClapTrap() : _name("Default"), _hitPoints(10), _energy(10), _attackDamage(0) {
	std::cout << "ClapTrap " << BOLD << GREEN << _name << RESET << " is created ! (default constructor)." << std::endl;
}

ClapTrap::ClapTrap(std::string newName) : _name(newName), _hitPoints(10), _energy(10), _attackDamage(0) {
	std::cout << "ClapTrap " << BOLD << GREEN << _name << RESET << " is created ! (name constructor)." << std::endl;
}

ClapTrap::ClapTrap(const ClapTrap& other) {
    std::cout << "ClapTrap " << BOLD << GREEN << _name << RESET << " is copied !" << std::endl;
	*this = other;
}

ClapTrap&   ClapTrap::operator=(const ClapTrap& other) {
	    std::cout << "ClapTrap " << BOLD << GREEN << _name << RESET << " is assigned !" << std::endl;
	if (this != &other)
	{
		_name = other._name;
...
ClapTrap::~ClapTrap() {
    std::cout << "ClapTrap " << BOLD << GREEN << _name << RESET << " is destroyed !" << std::endl;
}
